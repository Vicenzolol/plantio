...
| `swapForDate(iso, swaps)` | troca de turno do dia ou `null` |
...
```ts
{
  isWork: boolean;           // trabalha nesse dia (considera trocas)
  swap: ShiftSwap | null;    // troca registrada para o dia
  hasExtra: boolean;         // há horas extras avulsas no dia
  shiftHours: number;        // horas previstas do turno (0 se folga)
  extraHours: number;        // soma das horas extras avulsas do dia
}
```

Usado pelos componentes de agenda (`AgendaStrip`, `MonthCalendar`) para colorir e marcar cada dia.

## Onde isso é consumido no front

- **Dashboard** — `sumHours` (semana/mês), `isWorkDay` (status de hoje), `getDayStatus` (faixa de dias).
- **Horas** — `sumHours` por semana/mês/ano.
- **Agenda** — `getDayStatus` + `getMonthMatrix` (calendário mensal em grade).
